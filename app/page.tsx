...
export default function Page() {
  ...
  const handleCallApi = async (action: "analyzeATS" | "rewriteForJob") => {
    ...
    try {
      ...
      if (action === "analyzeATS") {
        ...
        // ✅ word_count من الـ backend أو نحسبه يدويًا
        if (typeof data.word_count === "number") {
          setRewrittenWordCount(data.word_count)
        } else if (rewritten) {
          const count = rewritten
            .trim()
            .split(/\s+/)
            .filter(Boolean).length
          setRewrittenWordCount(count)
        } else {
          setRewrittenWordCount(null)
        }
      }
  ...
  return (
    <main className="min-h-screen p-6 flex flex-col gap-6 bg-slate-100">
      <h1 className="text-2xl font-bold text-center mb-2">Smart ATS Resume Analyzer</h1>
      ...
      <div className="max-w-4xl mx-auto w-full space-y-4">
        <div>
          <label className="block text-sm font-semibold mb-1">Resume (Text)</label>
          <textarea
            value={resumeText}
        ...
        <div className="flex flex-wrap gap-3">
          ...
          <button
            ...
            disabled={loading}
            className={`px-4 py-2 rounded-md text-sm font-semibold text-white ${
              isBusy("rewrite") ? "bg-emerald-400" : "bg-emerald-600 hover:bg-emerald-700"
            }`}
          >
        ...
        {atsResult && (
          <div className="mt-4 border border-slate-200 rounded-md p-3 bg-white shadow-sm">
            <h2 className="font-semibold mb-2">ATS Result</h2>
            <p className="text-lg font-bold">Score: {atsResult.score} / 100</p>
            <p className="text-sm text-gray-700 mb-2">Match level: {atsResult.match_level}</p>
        ...
        {rewrittenResume && (
          <div className="mt-4 border border-slate-200 rounded-md p-4 bg-white shadow-sm">
            <div className="flex items-center justify-between mb-3">
              <h2 className="font-semibold">Rewritten Resume</h2>

              <div className="flex items-center gap-3">
                {rewrittenWordCount !== null && (
                  <span
                    className={`text-xs font-semibold px-2 py-1 rounded border ${
                      rewrittenWordCount >= 500 && rewrittenWordCount <= 700
                        ? "bg-emerald-50 text-emerald-700 border-emerald-200"
                        : "bg-amber-50 text-amber-700 border-amber-200"
                    }`}
                  >
                    Word Count: {rewrittenWordCount}
                  </span>
                )}
            ...
            {contactInfo && (contactInfo.fullName || contactInfo.email || contactInfo.phone) && (
              <div className="mb-3 p-3 bg-blue-50 border border-blue-200 rounded-md">
                ...
                <p className="text-sm text-blue-800 font-medium">{contactInfo.fullName}</p>
                ...
                {contactInfo.linkedin && (
                  <p className="text-xs text-blue-600">{contactInfo.linkedin}</p>
                )}
              </div>
            )}
